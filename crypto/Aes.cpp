...
//------------------------------------
void SubRoundKeyD(unsigned char * state, unsigned char * roundKey) {
	for (int i = 0; i < 16; i++) {
		state[i] ^= roundKey[i];
	}
}
 ...
 */
void InverseMixColumnsD(unsigned char * state) {
	unsigned char tmp[16];
...
void RoundD(unsigned char * state, unsigned char * key) {
	SubRoundKeyD(state, key);
	InverseMixColumnsD(state);
	ShiftRowsD(state);
	SubBytesD(state);
}

// Same as Round() but no InverseMixColumns
void InitialRoundD(unsigned char * state, unsigned char * key) {
	SubRoundKeyD(state, key);
	ShiftRowsD(state);
	SubBytesD(state);
}
...
void AESDecrypt(unsigned char * encryptedMessage, unsigned char * expandedKey, unsigned char * decryptedMessage)
{
	...
	InitialRoundD(state, expandedKey+160);
	...
	SubRoundKeyD(state, expandedKey); // Final round
...
int main() {
    ...
    // Assuming input is less than 16 bytes for simplicity. Padding logic would be needed for longer messages.
    unsigned char message[16] = {0};
    memcpy(message, input.c_str(), input.length());

    unsigned char encryptedMessage[16] = {0};
    unsigned char decryptedMessage[16] = {0};
    ...
    // Print decrypted message as string
    cout << "Decrypted message: ";
    for (int i = 0; i < 16; i++) {
        if (decryptedMessage[i] != 0) // Assuming null-terminated string
